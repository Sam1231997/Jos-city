...
const Hospital=()=>{
  ...
  const filteredEvents = filter === 'All' ? events.slice(16,20): events.filter(event => event.category === filter);
    return (
        <div>
        <div>
...
{/* BACKEND SIDE */}
    ...
    <div className="flex flex-wrap space-x-4 mb-4 md:space-x-4">
        {['All',  '4+ Rating', 'Price','Amenities','Property type','Parties'].map(category => (
          <button
            key={category}
